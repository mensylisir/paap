...
- [x] 完整验证 source 模式：源码仓 → Gitea mirror → Jenkins/kpack → registry/Harbor → GitOps 清单 → ArgoCD → 集群
  ...
  - 2026-06-29 复核：浏览器右侧栏显示 `gateway` 为 `源码交付 · 运行中`，源码仓库 `https://github.com/sqshq/piggymetrics.git`、分支 `master`、构建上下文 `.`、构建模块 `gateway`、构建镜像 `registry.piggymetrics-dev.paap.local/piggymetrics-dev/gateway:6bb2cf9`；PAAP Gitea 工作区列出 `piggymetrics-dev-components` GitOps 仓和 `piggymetrics-dev-gateway-source` 源码镜像仓，同源代理读取 `components/gateway` 成功返回 `Jenkinsfile`、`README.md`、`config.yaml`、`deployment.yaml`、`service.yaml`。
- [x] 完整验证 image 模式：镜像输入 → GitOps 清单 → ArgoCD → 集群
  ...
  - 2026-06-29 复核：浏览器右侧栏显示 `notification-service` 为 `镜像交付 · 运行中`，部署页反选 `本环境 · dev-registry · registry.piggymetrics-dev.paap.local`，配置页反选 `PiggyMetrics notification-service 运行变量`，Eureka/MongoDB/RabbitMQ 均反选本环境服务；PAAP 只读接口回读 `image/registryImage=registry.piggymetrics-dev.paap.local/piggymetrics-dev/notification-service:6bb2cf9`，Gitea 同源代理读取 `components/notification-service` 成功返回 `config.yaml`、`deployment.yaml`、`service.yaml`。
...
- [x] 代码层交付流程已拆为动作函数、步骤函数和流程函数：source 构建只准备 Gitea mirror/Jenkinsfile/README/Jenkins/kpack，不写 GitOps 清单；image 交付和已构建 source 交付由 PAAP 写部署清单、配置 ArgoCD 并持久化组件 GitOps 元数据
  ...
  - 2026-06-29 回归：`/home/mensyli1/.gvm/gos/go1.25.7/bin/go test -count=1 ./internal/service -run 'TestValidateComponent(SourceBuildPreflight|ImageDeploymentPreflight)|TestRunComponent(SourceBuildFlowRequiresGitAndCI|ImageDeliveryFlowRequiresGitAndCD|ImageDeliveryFlowPublishesGitOpsAndConfiguresArgoCD)|TestBuildComponentKpackSpec|TestPreferredSourceRegistryServiceType|TestPrepareComponentSourceBuildVersion|TestBuildComponentJenkinsfile|TestApplyComponentDeployVersion|Test(CreateComponent|UpdateComponent|DeployComponent|RunComponent)'` 通过；`npm --prefix frontend run test -- src/views/viewMarkup.test.ts src/views/EnvDetailView.test.ts src/views/serviceAssetConfig.test.ts src/views/componentConfigTemplateRuntime.test.ts src/views/componentWorkspace.test.ts src/utils/catalogGroups.test.ts src/components/workspaces/giteaRepository.test.ts --run` 通过。
