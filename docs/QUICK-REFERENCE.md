...
| 工具类型 | scope | 示例 |
|---------|-------|------|
| 数据库、缓存 | `tool-only` | PostgreSQL, Redis |
| 监控工具 | `environment-wide` | Prometheus, Grafana |
| CI/CD 工具 | `environment-wide` | ArgoCD, Tekton |
...
**Q: permissions.scope 应该选哪个？**
A: 如果工具需要访问环境内其他 namespace 的资源，选 `environment-wide`；否则选 `tool-only`。
