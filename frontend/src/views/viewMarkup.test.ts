...
describe('Vue view markup', () => {
  ...
  it('moves the shared resource pool entry to global navigation', async () => {
    ...
    expect(mainLayout.default).toContain('<span>平台插件</span>')
    expect(mainLayout.default).toContain('to="/platform/addons"')
  ...
  it('opens the platform shared resource pool through a dedicated overview route', async () => {
    ...
    expect(router.default).toContain('PlatformAddonsView')
    expect(router.default).toContain("path: 'platform/addons'")
  ...
  it('renders the service feature matrix from catalog data', async () => {
    ...
    expect(catalogView.default).not.toContain("label: '环境内创建'")
    expect(catalogView.default).not.toContain("label: '公共服务'")
  })
  ...
  it('renders catalog service details as a lifecycle and observability portal', async () => {
    ...
    expect(detailView.default).toContain('Grafana Dashboard')
    expect(detailView.default).toContain('catalog-grafana-frame')
    expect(detailView.default).toContain('embeddedGrafanaURL')
    expect(detailView.default).toContain('compactGrafanaEmbed')
    expect(detailView.default).toContain('withEmbeddedProxyAuthToken')
    expect(detailView.default).toContain('Loki 条件跳转')
    expect(detailView.default).toContain('打开 Loki')
    ...
    expect(detailView.default).not.toContain('metric-grid')
    expect(detailView.default).not.toContain('metric-tile')
  ...
  it('keeps service catalog monitor and log as separate tabs without an empty topology tab', async () => {
    ...
    expect(catalogDetail.default).toContain('Grafana Dashboard')
    expect(catalogDetail.default).toContain('iframe')
    expect(catalogDetail.default).toContain('catalog-grafana-grid')
    expect(catalogDetail.default).toContain('Loki 条件跳转')
    expect(catalogDetail.default).toContain('catalog-log-query')
    expect(catalogDetail.default).not.toContain("label: '监控日志'")
    expect(catalogDetail.default).not.toContain('应用依赖')
    expect(catalogDetail.default).not.toContain('metric-grid')
    expect(catalogDetail.default).not.toContain('metric-tile')
  })

  it('exposes HPA and KEDA autoscaling controls on a dedicated component scaling tab', async () => {
    const envDetail = await import('./EnvDetailView.vue?raw')
    const profileSource = await import('./componentProfile.ts?raw')

    expect(profileSource.default).toContain("{ key: 'autoscaling', label: '伸缩' }")
    expect(envDetail.default).toContain("configDrawerTab === 'autoscaling' && configDrawer.kind === 'component'")
    ...
    const deploySectionStart = envDetail.default.indexOf("configDrawerTab === 'deploy' && configDrawer.kind === 'component'")
    const scalingSectionStart = envDetail.default.indexOf("configDrawerTab === 'autoscaling' && configDrawer.kind === 'component'")
    const deploySection = envDetail.default.slice(deploySectionStart, scalingSectionStart)
    expect(deploySection).not.toContain('component-autoscaling-panel')
  })

  it('renders runtime metric charts from Prometheus time series instead of pod samples', async () => {
    const envDetail = await import('./EnvDetailView.vue?raw')

    expect(envDetail.default).toContain('runtimeMetricSeries')
    expect(envDetail.default).toContain('runtimeMetrics.value?.series')
    expect(envDetail.default).toContain('runtimeMetricChartForSeries')
    expect(envDetail.default).toContain('Prometheus 暂未返回当前卡片的时序数据。')
    expect(envDetail.default).toContain('正在读取当前卡片的运行指标和 Prometheus 时序')
