...
Three-process system communicating through Kubernetes CRDs and a shared PostgreSQL database:

```
Vue 3 Frontend  ──REST/WS──▶  PAAP Server (Gin)  ──CR CRUD──▶  PAAP Operator (controller-runtime)  ──▶  K8s
                                  │                                      │
                                  ├─ GORM models (PostgreSQL)     ├─ Application/Environment/ServiceInstance/Component controllers
...
| Database | `internal/database/` | DB init/connection (PostgreSQL only) |
...
**⑨ ServiceIP(需求9)** — ✅ 已弃用，以 Service FQDN `<service>.<namespace>.svc.cluster.local` 替代
- 组件 drawer 已展示 Service DNS 全名，包含 namespace+service 信息，比 ClusterIP 更实用
- 不再需要单独展示 ClusterIP / LoadBalancer IP
...
Week 5-7: ④(外部资源接入)→ ⑧(Ingress)   ← ④落地 External Capability 设计
...
| 9 | ServiceIP | — | ✅已弃用：在 Service FQDN 替代后移除 | 无 |
...
#### 允许创建的分支名：
领导让开发每一个特性或者修复某个bug都起个新分支，然后提交到远端，在远端创建pr才能合并到main
main, master, develop
feature/<kebab-case-name>（如 feature/user-auth）
bugfix/<kebab-case-name>
hotfix/<kebab-case-name>
release/<kebab-case-name>
support/<kebab-case-name>
版本号分支（如 1.x, 2.0, 3.1.0）
不合规的分支会被拒绝并返回提示：
ERROR: Branch name 'xxx' does not comply with naming convention.
