...
**⑤ 中间件版本号(需求5)** — 半成品,补界面即可
- 现状:`ServiceTemplate.ChartVersion` 字段已存在,后端安装时已用该版本。
- 缺口:前端安装/编辑中间件时无版本下拉,无法让用户选版本。
- 方案:在中间件安装表单加一个版本选择器,数据源 = 该 `ServiceType` 下所有 `ServiceTemplate` 的 `ChartVersion` 去重排序;选定后把 `chartVersion` 写进安装 payload。后端无需改。
- 工作量:**S(0.5–1 天)**。纯前端 + 少量 API。
...
Week 1-2: ⑤⑥(版本号+目录页)→ ⑦⑭(平台管理+角色)   ← 快速见效,领导能立刻看到
