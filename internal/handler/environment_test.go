...
func TestUpdateServiceDraftSavesValuesWithoutCreatingCR(t *testing.T) {
	...
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.PUT("/api/v1/environments/:id/services/:serviceId", withTestAuthUser(2, UpdateService))
...
func TestUpdateServiceRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	inst := model.ServiceInstallation{EnvironmentID: env.ID, ServiceType: "redis", ServiceName: "redis", Status: "draft", Values: `{"architecture":"standalone"}`}
	if err := db.Create(&inst).Error; err != nil {
		t.Fatalf("create service installation: %v", err)
	}

	path := fmt.Sprintf("/api/v1/environments/%d/services/%d", env.ID, inst.ID)
	req := httptest.NewRequest(http.MethodPut, path, strings.NewReader(`{"values":{"architecture":"replication"}}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	...
	var saved model.ServiceInstallation
	if err := db.First(&saved, inst.ID).Error; err != nil {
		t.Fatalf("load service installation: %v", err)
	}
	if saved.Values != inst.Values {
		t.Fatalf("values changed for rejected update: got %s want %s", saved.Values, inst.Values)
	}
}

func TestUpdateServiceRejectsNonMembersBeforeServiceLookup(t *testing.T) {
	previousDB := database.DB
	previousClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
...
func TestUpdateServiceChecksTemplateAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	previousClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	inst := model.ServiceInstallation{EnvironmentID: env.ID, ServiceType: "missing-template", ServiceName: "missing", Status: "draft"}
	if err := db.Create(&inst).Error; err != nil {
		t.Fatalf("create service installation: %v", err)
	}

	path := fmt.Sprintf("/api/v1/environments/%d/services/%d", env.ID, inst.ID)
	req := httptest.NewRequest(http.MethodPut, path, strings.NewReader(`{"values":{"enabled":"true"}}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
...
func TestCreateEnvironmentAppliesTemplateResourceQuota(t *testing.T) {
	previousDB := database.DB
	previousClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	sqlDB.SetMaxOpenConns(1)
	if err := db.AutoMigrate(&model.Application{}, &model.Environment{}, &model.EnvTemplate{}, &model.ServiceTemplate{}); err != nil {
		t.Fatalf("migrate: %v", err)
	}
	database.DB = db
	...
	body, _ := json.Marshal(CreateEnvRequest{Name: "预发环境", Identifier: "stage", TemplateID: tmpl.ID})
	req := httptest.NewRequest(http.MethodPost, "/api/v1/applications/1/environments", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	...
	if envCR.Spec.ResourceQuota == nil {
		t.Fatalf("resourceQuota is nil, want template quota")
	}
	if envCR.Spec.ResourceQuota.CPU != "6核" || envCR.Spec.ResourceQuota.Memory != "12GB" || envCR.Spec.ResourceQuota.Storage != "80GB" {
		t.Fatalf("resourceQuota = %#v", envCR.Spec.ResourceQuota)
	}
...
func TestSetComponentNodePortAccessRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	path := fmt.Sprintf("/api/v1/environments/%d/components/%d/nodeport-access", env.ID, comp.ID)
	req := httptest.NewRequest(http.MethodPut, path, strings.NewReader(`{"enabled":true}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	...
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.PUT("/api/v1/environments/:id/components/:componentId/nodeport-access", withTestAuthUser(2, SetComponentNodePortAccess))
	router.ServeHTTP(rec, req)
...
func TestSetComponentNodePortAccessChecksNamespaceAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	previousClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
...
func TestSetComponentNodePortAccessRejectsNonMembersBeforeComponentLookup(t *testing.T) {
	previousDB := database.DB
	t.Cleanup(func() { database.DB = previousDB })
	...
	path := fmt.Sprintf("/api/v1/environments/%d/components/999/nodeport-access", env.ID)
	req := httptest.NewRequest(http.MethodPut, path, strings.NewReader(`{"enabled":true}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
...
func TestGetServiceInstanceRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/environments/1/services/1", nil)
	rec := httptest.NewRecorder()
...
func TestGetServiceInstanceRejectsNonMembersBeforeServiceLookup(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/999", env.ID), nil)
	rec := httptest.NewRecorder()
...
func TestGetServiceRuntimeMetricsRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	service := model.ServiceInstallation{EnvironmentID: env.ID, ServiceType: "git", Status: "running", Namespace: "hidden-prod-git"}
	if err := db.Create(&service).Error; err != nil {
		t.Fatalf("create service: %v", err)
	}

	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/%d/runtime-metrics", env.ID, service.ID), nil)
	rec := httptest.NewRecorder()

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.GET("/api/v1/environments/:id/services/:serviceId/runtime-metrics", withTestAuthUser(2, GetServiceRuntimeMetrics))
	router.ServeHTTP(rec, req)
...
func TestGetServiceRuntimeMetricsRejectsNonMembersBeforeServiceLookup(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/999/runtime-metrics", env.ID), nil)
	rec := httptest.NewRecorder()
...
func TestGetComponentRuntimeMetricsRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/components/%d/runtime-metrics", env.ID, comp.ID), nil)
	rec := httptest.NewRecorder()

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.GET("/api/v1/environments/:id/components/:componentId/runtime-metrics", withTestAuthUser(2, GetComponentRuntimeMetrics))
	router.ServeHTTP(rec, req)
...
func TestGetComponentRuntimeMetricsRejectsNonMembersBeforeComponentLookup(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/components/999/runtime-metrics", env.ID), nil)
	rec := httptest.NewRecorder()
...
func TestGetComponentRuntimeMetricsChecksComponentAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	env := model.Environment{ApplicationID: app.ID, Name: "测试环境", Identifier: "staging", Namespace: "test-staging"}
	if err := db.Create(&env).Error; err != nil {
		t.Fatalf("create env: %v", err)
	}
	if err := db.Create(&model.AppMember{ApplicationID: app.ID, UserID: 2, Role: "member"}).Error; err != nil {
...
func TestGetComponentRuntimeLogsRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/components/%d/runtime-logs", env.ID, comp.ID), nil)
	rec := httptest.NewRecorder()
...
func TestGetComponentRuntimeLogsRejectsNonMembersBeforeComponentLookup(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
...
func TestGetComponentRuntimeLogsChecksComponentAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
...
func TestProxyComponentRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/components/%d/proxy/", env.ID, comp.ID), nil)
	rec := httptest.NewRecorder()

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.Any("/api/v1/environments/:id/components/:componentId/proxy/*path", withTestAuthUser(2, ProxyComponent))
	router.ServeHTTP(rec, req)
...
func TestProxyComponentRejectsNonMembersBeforeComponentLookup(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/components/999/proxy/", env.ID), nil)
	rec := httptest.NewRecorder()
...
func TestProxyComponentChecksComponentAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
...
func TestHandleComponentConsoleRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/components/%d/console", env.ID, comp.ID), nil)
	rec := httptest.NewRecorder()

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.GET("/api/v1/environments/:id/components/:componentId/console", withTestAuthUser(2, HandleComponentConsole))
	router.ServeHTTP(rec, req)
...
func TestHandleComponentConsoleRejectsNonMembersBeforeComponentLookup(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/components/999/console", env.ID), nil)
	rec := httptest.NewRecorder()
...
func TestHandleComponentConsoleChecksComponentAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
...
func TestGetServiceRuntimeLogsRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/%d/runtime-logs", env.ID, service.ID), nil)
	rec := httptest.NewRecorder()

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.GET("/api/v1/environments/:id/services/:serviceId/runtime-logs", withTestAuthUser(2, GetServiceRuntimeLogs))
	router.ServeHTTP(rec, req)
...
func TestGetServiceRuntimeLogsRejectsNonMembersBeforeServiceLookup(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/999/runtime-logs", env.ID), nil)
	rec := httptest.NewRecorder()
...
func TestProxyServiceInstanceRejectsNonMembers(t *testing.T) {
	previousDB := database.DB
	t.Cleanup(func() { database.DB = previousDB })
	...
	service := model.ServiceInstallation{EnvironmentID: env.ID, ServiceType: "custom", Status: "running", Namespace: "hidden-prod-custom"}
	if err := db.Create(&service).Error; err != nil {
		t.Fatalf("create service: %v", err)
	}

	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/%d/proxy/", env.ID, service.ID), nil)
	rec := httptest.NewRecorder()

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.Any("/api/v1/environments/:id/services/:serviceId/proxy/*path", withTestAuthUser(2, ProxyServiceInstance))
	router.ServeHTTP(rec, req)
...
func TestProxyServiceInstanceRejectsNonMembersBeforeServiceLookup(t *testing.T) {
	previousDB := database.DB
	t.Cleanup(func() { database.DB = previousDB })
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/999/proxy/", env.ID), nil)
	rec := httptest.NewRecorder()
...
func TestProxyServiceInstanceChecksServiceAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	t.Cleanup(func() { database.DB = previousDB })
...
func TestHandleServiceConsoleRejectsNonMembers(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/%d/console", env.ID, service.ID), nil)
	rec := httptest.NewRecorder()

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.GET("/api/v1/environments/:id/services/:serviceId/console", withTestAuthUser(2, HandleServiceConsole))
	router.ServeHTTP(rec, req)
...
func TestHandleServiceConsoleRejectsNonMembersBeforeServiceLookup(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/environments/%d/services/999/console", env.ID), nil)
	rec := httptest.NewRecorder()
...
func TestHandleServiceConsoleChecksServiceAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	previousK8sClient := k8s.GetClient()
	t.Cleanup(func() {
		database.DB = previousDB
...
func TestDeployComponentRejectsNonMembers(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, fmt.Sprintf("/api/v1/components/%d/deploy", comp.ID), strings.NewReader(`{"version":"v2"}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.POST("/api/v1/components/:id/deploy", withTestAuthUser(2, DeployComponent))
	router.ServeHTTP(rec, req)
	...
	if saved.Version != "v1" || saved.Status != "draft" {
		t.Fatalf("component changed for non-member: version=%q status=%q", saved.Version, saved.Status)
	}
}

func TestDeployComponentRejectsNonMembersBeforeVersionValidation(t *testing.T) {
	previousDB := database.DB
	t.Cleanup(func() { database.DB = previousDB })
	...
	req := httptest.NewRequest(http.MethodPost, fmt.Sprintf("/api/v1/components/%d/deploy", comp.ID), strings.NewReader(`{"version":""}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
...
func TestDeployComponentValidatesVersionAfterMemberAccess(t *testing.T) {
	previousDB := database.DB
	t.Cleanup(func() { database.DB = previousDB })
